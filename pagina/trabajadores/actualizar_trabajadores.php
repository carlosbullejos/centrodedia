...
$stmt = $conexion->prepare("
  SELECT nombre, apellidos, dni, telefono, puesto, fecha_contratacion, especialidad, fecha_nacimiento
    FROM trabajadores
    WHERE id = ?
");
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->bind_result($nombre, $apellidos, $dni, $telefono, $puesto, $fecha_contratacion, $especialidad, $fecha_nacimiento);
$stmt->fetch();
$stmt->close();
$conexion->close();
?>
...
<center>
  ...
  <form action="actualizar_trabajadores2.php" method="post" class="formulario">
    ...
    <div class="form-group">
      ...
      <input type="text" id="puesto" name="puesto" required value="<?= htmlspecialchars($puesto) ?>">
    </div>

    <div class="form-group">
      <label for="fecha_contratacion">Fecha de contratación:</label>
      <input type="date" id="fecha_contratacion" name="fecha_contratacion" required value="<?= htmlspecialchars($fecha_contratacion) ?>">
    </div>

    <div class="form-group">
      <label for="especialidad">Especialidad:</label>
      <input type="text" id="especialidad" name="especialidad" required value="<?= htmlspecialchars($especialidad) ?>">
    </div>
