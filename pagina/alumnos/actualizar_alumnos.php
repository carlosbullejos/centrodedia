...
<?php
    ...
    $id = intval($_GET['id']);
    $stmt = $conexion->prepare("SELECT nombre, apellidos, dni, fecha_nacimiento, telefono, direccion, localidad, fecha_ingreso, profesor_id FROM alumnos WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->bind_result($nombre, $apellidos, $dni, $fecha_nacimiento, $telefono, $direccion, $localidad, $fecha_ingreso, $profesor_id);
    $stmt->fetch();
    $stmt->close();
    $conexion->close();
?>
...
<center>
    ...
    <form action="actualizar_alumnos2.php" method="post" class="formulario">
        ...
        <div class="form-group">
            ...
            <input type="text" id="dni" name="dni" maxlength="9" required value="<?= htmlspecialchars($dni) ?>">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="number" id="profesor_id" name="profesor_id" required value="<?= htmlspecialchars($profesor_id) ?>">
